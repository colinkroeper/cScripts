...
public class NPCInteraction implements Loggable {
    ...
    private static final int
            ...
            NPC_SHOWING_ITEM_INTERFACE_WINDOW = 1, // i added this
            LEVEL_UP_SHOWING_NEW_ITEMS = 11,// i added this
            PLAYER_TALKING_INTERFACE_WINDOW = 217,
            SELECT_AN_OPTION_INTERFACE_WINDOW = 219,
            LEVEL_UP_INTERFACE = 233, // i added this
            SINGLE_OPTION_DIALOGUE_WINDOW = 229;

    private static final int[] ALL_WINDOWS = new int[]{ITEM_ACTION_INTERFACE_WINDOW, LEVEL_UP_SHOWING_NEW_ITEMS, LEVEL_UP_INTERFACE, NPC_SHOWING_ITEM_INTERFACE_WINDOW, NPC_TALKING_INTERFACE_WINDOW, PLAYER_TALKING_INTERFACE_WINDOW, SELECT_AN_OPTION_INTERFACE_WINDOW, SINGLE_OPTION_DIALOGUE_WINDOW};
    ...
    public static void handleConversationContains(String... options) {
        ...
        while (limit++ < 50) {
            ...
            for (RSInterface selected : selectableOptions) {
                ...
                General.sleep(General.randomSD(350, 2250, 775, 350));
                General.println("[Debug]: Replying with option: " + selected.getText());
    ...
    private static List<RSInterface> getAllOptions(String regex) {
        List<RSInterface> list = getConversationDetails();
        return list != null ? list.stream().filter(rsInterface -> rsInterface.getText().toLowerCase().matches(regex.toLowerCase())).collect(Collectors.toList()) : null;
    ...
    private static List<RSInterface> getAllOptions(String... options) {
        final List<String> optionList = Arrays.stream(options).map(String::toLowerCase)
                .collect(Collectors.toList());
        List<RSInterface> list = getConversationDetails();
